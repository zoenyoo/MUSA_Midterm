...
reg1 <- lm(price ~ ., data = st_drop_geometry(boulder.sf) %>% 
                             dplyr::select(price, Age, designCode, qualityCode, ConstCode,
...
**Coefficient plots**
```{r effect_plots, cache = TRUE, message = FALSE, warning = FALSE}
...
reg.nhood <- lm(price ~ ., data = as.data.frame(boulder.training) %>% 
                                  dplyr::select(GEOID, price, price, Age, qualityCode, 
                                                carStorageSF, nbrBedRoom, TotalFinishedSF,
                                                playground_nn3))
